...
import { Plus, Search, Pencil, Trash2, X, Users, ClipboardList, ChevronDown, ChevronUp, Pill, Calendar, CalendarPlus } from 'lucide-react'
...
// ── Modal: السجل الطبي ────────────────────────────────────────────
function HistoryModal({ patient, onClose, headers, API, role }) {
      ...
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-[0_24px_60px_rgba(0,0,0,0.12)]">
        ...
        <div className="flex justify-between items-center p-5 sm:p-6 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            {/* زرار إضافة موعد سريع — Doctor فقط */}
            {role === 'Doctor' && (
              <button
                onClick={() => {
                  onClose()
                  navigate('/appointment', { state: { quickPatient: patient } })
                }}
                title="إضافة موعد سريع"
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#0A4174] text-white text-xs font-bold border-none cursor-pointer hover:bg-[#000340] transition-colors">
