...
import { Plus, Trash2, X, BadgeCent, ChevronDown, Search, HandCoins } from 'lucide-react'
...
export default function Payment() {
  ...
  const [showSummary, setShowSummary] = useState(null) // 'total' | 'paid' | 'remaining'
  ...
  async function loadSummary() {
    try {
      const { data } = await axios.get(`${API_BASE}/api/payments/summary`, { headers })
      setSummary(data)
    } catch (e) { console.error(e) }
  }
  ...
  useEffect(() => { loadPatients(); loadSummary() }, [])
  ...
  const visibleCount    = (role === 'Admin' || maxPercent >= 100) ? patients.length : (patients.length ? Math.round(patients.length * (maxPercent / 100)) : 0)
  const visiblePatients = search.trim() ? patients : patients.slice(0, visibleCount)
  const filteredPatients = visiblePatients.filter(p => p.full_name?.includes(search) || p.phone_number?.includes(search))
  ...
  return (
    <div dir="rtl" className="mt-5 lg:mt-0 min-h-screen bg-white p-4 sm:p-6 lg:p-8 font-[Tajawal,Cairo,sans-serif]">
      ...
      {/* ── ملخص الأرقام — 3 cards قابلة للضغط ── */}
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          { key: 'total',     label: 'إجمالي', value: visibleTotals.total,     color: 'text-[#000340]' },
          { key: 'paid',      label: 'مدفوع',  value: visibleTotals.paid,      color: 'text-green-600' },
          { key: 'remaining', label: 'متبقي',  value: visibleTotals.remaining, color: visibleTotals.remaining > 0 ? 'text-red-500' : 'text-green-500' },
        ].map(s => (
          <button key={s.key} onClick={() => { loadSummary(); setShowSummary(s.key) }}
            className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm text-center cursor-pointer hover:border-[#4685AC] hover:shadow-md transition-all">
        ...
        <div className="fixed inset-0 z-50 bg-[#000340]/30 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl">
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100">
              <div>
                <h3 className="text-[#000340] font-bold text-base m-0">
                  {showSummary === 'total'     && 'إجمالي جميع المرضى'}
                  {showSummary === 'paid'      && 'الدفعات المستلمة اليوم'}
                  {showSummary === 'remaining' && 'المبالغ المتبقية'}
                </h3>
                <p className="text-gray-400 text-xs mt-0.5">
                  {showSummary === 'total'     && `${Number(summary.summary.total_all).toLocaleString('ar-JO')} د.أ`}
                  {showSummary === 'paid'      && `${Number(summary.summary.paid_today).toLocaleString('ar-JO')} د.أ اليوم`}
                  {showSummary === 'remaining' && `${Number(summary.summary.remaining_all).toLocaleString('ar-JO')} د.أ`}
                </p>
              </div>
              <button onClick={() => setShowSummary(null)} className="text-gray-400 bg-transparent border-none cursor-pointer hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              {showSummary === 'paid' && (
                summary.details.paid_today.length === 0
                  ? <p className="text-gray-300 text-sm text-center py-8">لا توجد دفعات اليوم</p>
                  : <div className="flex flex-col gap-2">
                      {summary.details.paid_today.map((p, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-green-50 border border-green-100">
                          <div>
                            <p className="text-[#000340] text-sm font-semibold">{p.full_name}</p>
                            <p className="text-gray-400 text-xs mt-0.5">{p.payment_date?.slice(0, 10)} — {p.payment_method}</p>
                          </div>
                          <p className="text-green-600 font-bold text-sm">{Number(p.amount).toLocaleString('ar-JO')} د.أ</p>
                        </div>
                      ))}
                    </div>
              )}
              {showSummary === 'remaining' && (
                summary.details.remaining.length === 0
                  ? <p className="text-gray-300 text-sm text-center py-8">لا توجد مبالغ متبقية</p>
                  : <div className="flex flex-col gap-2">
                      {summary.details.remaining.map((p, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-red-50 border border-red-100">
                          <div>
                            <p className="text-[#000340] text-sm font-semibold">{p.full_name}</p>
                            <p className="text-gray-400 text-xs mt-0.5">الإجمالي: {Number(p.total_amount).toLocaleString('ar-JO')} د.أ</p>
                          </div>
                          <p className="text-red-500 font-bold text-sm">{Number(p.remaining_amount).toLocaleString('ar-JO')} د.أ</p>
                        </div>
                      ))}
                    </div>
              )}
              {showSummary === 'total' && (
                summary.details.total.length === 0
                  ? <p className="text-gray-300 text-sm text-center py-8">لا توجد بيانات</p>
                  : <div className="flex flex-col gap-2">
                      {summary.details.total.map((p, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-gray-50 border border-gray-100">
                          <div>
                            <p className="text-[#000340] text-sm font-semibold">{p.full_name}</p>
                            <p className="text-gray-400 text-xs mt-0.5">مدفوع: {Number(p.paid_amount).toLocaleString('ar-JO')} د.أ</p>
                          </div>
                          <p className="text-[#000340] font-bold text-sm">{Number(p.total_amount).toLocaleString('ar-JO')} د.أ</p>
                        </div>
                      ))}
                    </div>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm flex flex-col"
          ...
          <div className="overflow-y-auto" style={{ maxHeight: '55vh' }}>
            {filteredPatients.length === 0 && <p className="text-gray-300 text-xs text-center py-8">لا يوجد نتائج</p>}
            ...
            <>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                ...
                <div className="mt-3 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-[#4685AC] to-[#3F7E5C] rounded-full transition-all"
                    style={{ width: `${selectedPat.total_amount ? Math.min(100, ((selectedPat.total_amount - selectedPat.remaining_amount) / selectedPat.total_amount) * 100) : 0}%` }} />
                </div>
              </div>
                      ...
                      <tbody>
                        {payments.map(p => (
                          <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 last:border-0">
                            <td className="px-4 py-3 text-green-600 font-bold text-sm">{Number(p.amount).toLocaleString('ar-JO')} د.أ</td>
        ...
        <div className="fixed inset-0 z-50 bg-[#000340]/30 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex flex-col gap-3">
              <div>
                <label className="text-gray-600 text-xs block mb-1">المبلغ * (متبقي: {Number(selectedPat.remaining_amount).toLocaleString('ar-JO')} د.أ)</label>
                ...
                {formik.errors.amount && formik.touched.amount && <p className="text-red-400 text-xs mt-1">{formik.errors.amount}</p>}
              </div>
              <div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={() => setShowModal(false)} className="px-5 py-2.5 rounded-xl border border-gray-200 bg-transparent text-gray-600 cursor-pointer text-sm">إلغاء</button>
            </div>
          </div>
        </div>
      )}
