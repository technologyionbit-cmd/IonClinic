...
export default function Inventory() {
  ...
  const [addForm,  setAddForm]  = useState({ name: '', initial_quantity: '', cost_price: '', usage_price: '', alert_threshold_percent: 20 })
  const [editForm, setEditForm] = useState({ name: '', cost_price: '', usage_price: '', alert_threshold_percent: 20, added_quantity: '' })
  ...
  // PUT  /:id       → Admin
  // DELETE /:id     → Admin
  // POST /consume   → Admin, Doctor  ← لكن يشترط patient_id + appointment_id
  //                   لذا الخصم يصير فقط من صفحة المواعيد (PUT /:id/record)
  // GET  /          → Admin, Doctor, Secretary
  let role = 'Secretary'
  try {
    if (token) { const p = JSON.parse(atob(token.split('.')[1])); role = p.role || 'Secretary' }
  } catch {}

  const canManage = role === 'Admin'   // إضافة / تعديل / حذف
  ...
  async function handleAdd() {
    if (!addForm.name || !addForm.initial_quantity || !addForm.cost_price || !addForm.usage_price) {
    ...
    try {
      await axios.post(API, {
        ...
        usage_price:             Number(addForm.usage_price),
        alert_threshold_percent: Number(addForm.alert_threshold_percent) || 20,
      }, { headers })
      showToastMsg('success', 'تم إضافة المادة')
      await load()
      setShowAdd(false)
      setAddForm({ name: '', initial_quantity: '', cost_price: '', usage_price: '', alert_threshold_percent: 20 })
    } catch (e) { showToastMsg('error', e.response?.data?.message || 'حدث خطأ') }
    finally { setSaving(false) }
  }

  // ── Edit (يدعم إعادة التخزين عبر added_quantity) ─────────────
  async function handleEdit() {
    if (!editForm.name || !editForm.cost_price || !editForm.usage_price) {
      showToastMsg('error', 'يجب ملء جميع الحقول'); return
    }
    setSaving(true)
    try {
      await axios.put(`${API}/${editTarget.id}`, {
        ...
        usage_price:             Number(editForm.usage_price),
  ...
  return (
    <div dir="rtl" className="mt-5 lg:mt-0 min-h-screen bg-white p-4 sm:p-6 lg:p-8 font-[Tajawal,Cairo,sans-serif]">
      ...
      {/* ملاحظة: الخصم يصير من صفحة المواعيد */}
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {items.map(item => {
            ...
            // ربح الأدمن فقط
            const profit   = (item.usage_price - item.cost_price) * (item.initial_quantity - item.current_quantity)
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden w-full max-w-xs">
                      <div
                        className={`h-full rounded-full transition-all ${critical ? 'bg-red-500' : warning ? 'bg-amber-400' : 'bg-green-400'}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  ...
                  <div className="text-right flex-shrink-0 space-y-1.5">
                    {/* ربح — Admin فقط */}
                    {canManage && (
                      <p className="text-green-600 text-xs font-bold">ربح: {Number(profit).toLocaleString('ar-JO')} د.أ</p>
                    )}
                    <p className="text-gray-400 text-xs">تكلفة: {item.cost_price} | استخدام: {item.usage_price}</p>
                    ...
                    {canManage && (
                      <div className="flex gap-1.5 justify-end">
                        <button
                          onClick={() => {
                            setEditTarget(item)
                            setEditForm({ name: item.name, cost_price: item.cost_price, usage_price: item.usage_price, alert_threshold_percent: item.alert_threshold_percent, added_quantity: '' })
                            setShowEdit(true)
                          }}
                        ...
                        <button
                          onClick={() => { setDelTarget(item); setShowDel(true) }}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {[
                ...
                ['cost_price',       'سعر التكلفة (د.أ) *'],
                ['usage_price',      'سعر الاستخدام (د.أ) *'],
        ...
        <div className="fixed inset-0 z-50 bg-[#000340]/30 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl">
            ...
            <p className="text-gray-500 text-sm mb-5">
              هل تريد حذف <span className="text-[#000340] font-semibold">{delTarget?.name}</span>؟
            </p>
