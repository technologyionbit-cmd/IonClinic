...
import {
  ...
  Filter, Search, Clock, AlertCircle, ClipboardList, Pill
} from 'lucide-react'
...
export default function Appointment() {
  ...
  const [loading,         setLoading]         = useState(true)   // ✅ جوا الكومبوننت
  const [saving,          setSaving]          = useState(false)
...
const API_BASE = import.meta.env.VITE_API_URL || ''
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        setShowModal(false); setEditTarget(null); formik.resetForm(); setPatientSearch('')
  ...
  function openAdd()    { setEditTarget(null); formik.resetForm(); setPatientSearch(''); setBackendError(''); setTimeWarning(''); setShowModal(true) }
  function openEdit(a)  { setEditTarget(a); setBackendError(''); setTimeWarning(''); setShowModal(true) }
  function closeModal() { setShowModal(false); setEditTarget(null); formik.resetForm(); setPatientSearch(''); setBackendError(''); setTimeWarning('') }
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <div key={a.id}
                  className="grid gap-2 px-5 py-[14px] items-center hover:bg-gray-50 transition-colors"
                  style={{
                    gridTemplateColumns: '2fr 1.8fr 0.7fr 1fr 0.8fr 1fr 96px',
                    borderBottom: i < filtered.length - 1 ? '1px solid #f3f4f6' : 'none'
                  }}>
                  <span className="text-[#000340] text-sm font-medium truncate">{a.full_name}</span>
                  ...
                  <span className="text-gray-500 text-sm">{a.phone_number}</span>
